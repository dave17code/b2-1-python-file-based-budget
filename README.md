...
wc -l \
  ...
  data/budgets.jsonl \
  import_test_data/transactions.jsonl
```
...
JSONL 파일의 중간 한 줄을 직접 수정하거나 삭제하지 않습니다.

```text
기존 transactions.jsonl 읽기
        ↓
수정 또는 삭제 결과를 임시 파일에 작성
        ↓
임시 파일 flush 및 fsync
        ↓
os.replace()로 기존 파일과 교체
```

이 방식은 수정·삭제 도중 프로그램이 중단되어 원본 파일이 일부만 남는 문제를 줄입니다.

---

# 📄 CSV 스키마

가져오기와 내보내기는 다음 컬럼을 사용합니다.
...
```csv
date,type,category,amount,memo,tags
2026-07-01,income,salary,3000000,7월 급여,"salary,monthly"
2026-07-02,expense,rent,850000,7월 원룸 월세,"housing,monthly,fixed"
2026-07-06,expense,food,220000,7월 식료품 장보기,"groceries,home"
```
...
|---|---:|
| 본 데이터 폴더 | `data/` |
| CSV 가져오기 검증 폴더 | `import_test_data/` |
| 기본 카테고리 | 8개 |
| 예산 데이터 | 2건 |
| 본 거래 데이터 | 20건 |
| 7월 거래 | 10건 |
...
| 7월 CSV | 11줄 |
| 8월 CSV | 11줄 |
| 정상 CSV 가져오기 | `imported=10, skipped=0` |
| 가져온 거래 | 10건 |
| 정상 종료 코드 | `0` |
| 입력 오류 종료 코드 | `1` |
| 사용자 중단 종료 코드 | `130` |
